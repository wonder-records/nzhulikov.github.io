(function($) {
	...
	*/
	var doSearch = function(options, callback) {
		...
		var url = 'https://api.spotify.com/v1/search?type=track&limit='
		 + options.maxTracks + '&offset=' + offset + '&q=' + words.join('%20');
	...
	*/
	$.getRandomSongs = function(options, callback) {
		...
		doSearch(options, function(response) {
			if(response.tracks.total === 0 || response.tracks.items.length === 0) return $.getRandomSongs(options, callback);
			return callback(response.tracks.items);
		});
	};
